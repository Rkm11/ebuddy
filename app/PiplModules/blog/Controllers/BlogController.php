...
class BlogController extends Controller
{
	...
	{
		$posts = Post::translatedIn(\App::getLocale())->orderBy('updated_at','desc')->get();
		
		$existing_categories = PostCategory::withTranslation()->get();
						
		$tree = $this->getCategoryTreeList($existing_categories->toTree(),'<li>',true);

		return view('blog::posts',array('posts'=>$posts,'category_tree'=> $tree));
	}
	
	public function searchPost($keyword)
	{
			$matching_posts =	Post::search(array('short_description','title','description'), "%$keyword%")->get();
