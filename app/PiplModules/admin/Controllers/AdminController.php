...
class AdminController extends Controller
{
     ...
     protected function validator(Request $request)
    {
       ...
       public function showLogin()
       {
	    ...
	    $registered_users =  $all_users->reject(function ($user) {   return $user->user->hasRole('registered.user') === false; });
	    $admin_users =  $all_users->reject(function ($user) {   return $user->user->hasRole('registered.user') === true; });
               ...
               if($validate_response->fails())
                {
                    ...
                    if($user_password_chk)
                    {
                            ...
                            $errorMsg  = "Error! current entered password is not valid.";
                            return redirect("admin/update-admin-user/".$user_id)->with("password-update-fail",$errorMsg);
                   }
             }
        }else{
                $errorMsg  = "Error! Something wrong is going on.";
  ...
  protected function verifyUserEmail($activation_code)
   ...
   protected function updateRegisteredUserPasswordInfo(Request $data,$user_id)
	...
	{
                  $role_list= Role::all();
                  $role_listing=$role_list->reject(function ($role){ return ($role->slug=="superadmin")==true; });        
		...
		if($request->method() == "GET" )
		 {
				...
				$filtered_reg_role = $all_roles->filter(function($value,$key){ return $value->slug=='registered.user'; })->first();
